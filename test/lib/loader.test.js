...
describe('Loader', () => {
    describe('loadRulesets()', () => {
        test('load default rules', () => {
            return expect(loader.loadRulesets(['default'])).resolves.toEqual([
                'default',
            ]);
        });

        test('load strict rules', () => {
            return expect(loader.loadRulesets(['strict'])).resolves.toEqual([
                'strict',
                'default',
            ]);
        });
        ...
        test('load default & strict rules', () => {
            return expect(
                loader.loadRulesets(['strict', 'default'])
            ).resolves.toEqual(['strict', 'default']);
        });

        describe('when loading from a local file', () => {
            test('retrieves rules from the file', () => {
                const file = __dirname + '/../../rules/strict.yaml';
                return expect(loader.loadRulesets([file])).resolves.toEqual([
                    file,
                    'default',
                ]);
            });
        ...
        describe('when loading from url', () => {
            ...
            test('retrieves rules from valid url', () => {
                ...
                return expect(
                    loader.loadRulesets([host + '/'])
                ).resolves.toEqual([host + '/', 'default']);
            });
    ...
    describe('loadSpec()', () => {
        ...
        test('resolves refs when passed { resolve: true }', async () => {
            ...
            expect(spec.paths['/a'].post.description).toEqual(
        ...
        test('resolves JSON Schema $refs when passed { filters: [ jsonSchema ] }', async () => {
            ...
            const spec = await loader.loadSpec(
                samplesDir + 'json-schema/openapi.yaml',
                {
                    filters,
                    resolve: true,
                }
