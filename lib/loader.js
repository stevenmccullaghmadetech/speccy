...
const readOrError = async (file, options = {}) => {
    try {
        ...
        if (error instanceof OpenError) {
            ...
            console.error(
                'Could not read YAML/JSON from file: ' + error.message
            );
        } else {
...
const recursivelyLoadRulesets = async (ruleset, loadedRulesets, options) => {
    ...
    try {
        ...
        if (typeof data.require == 'string') {
            const requiredLoadedFiles = await recursivelyLoadRulesets(
                data.require,
                loadedRulesets,
                options
            );
        ...
        if (data.rules) {
            if (verbose > 1) {
                console.log(
                    `Found ${
                        data.rules.length
                    } rules in ${ruleset}: ${data.rules.map((x) => x.name)}`
                );
            }
...
const loadRulesets = async (loadFiles, options = {}) => {
    const { verbose } = options;
    const rulesetList =
        loadFiles && loadFiles.length > 0 ? loadFiles : ['default'];
    const allDependencies = await asyncMap(rulesetList, (ruleset) =>
        recursivelyLoadRulesets(ruleset, [], { verbose })
    );
        ...
        .then(
            async (unresolved) => {
                ...
                if (options.resolve === true) {
                    resolved = (await resolveContent(unresolved, options))
                        .openapi;
                }
                return resolved;
            },
